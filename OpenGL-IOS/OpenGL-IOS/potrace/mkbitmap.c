...
static void process_file(FILE *fin, FILE *fout, const char *infile, const char *outfile) {
  ...
  for (count=0; ; count++) {
    ...
    case 1:  /* unexpected end of file */
      fprintf(stderr, ": %s: warning: premature end of file\n", infile);
      break;
    }
...
static void dopts(int ac, char *av[]) {
  ...
  while ((c = getopt_long(ac, av, shortopts, longopts, NULL)) != -1) {
    switch (c) {
    case 'h':
      fprintf(stdout, " Transforms images into bitmaps with scaling and filtering.\n\n");
      usage(stdout);
      exit(0);
      break;
    case 'v':
    ...
    case 'f':
      ...
      if (*p || info.lambda<0) {
	fprintf(stderr, "invalid filter radius -- %s\n", optarg);
        exit(1);
      }
      break;
    case 'n':
