...
static void usage(FILE *f) {
  ...
  fprintf(f, "Usage: " " [options] [filename...]\n");
...
static void dopts(int ac, char *av[]) {
  ...
  while ((c = getopt_long(ac, av, shortopts, longopts, NULL)) != -1) {
    switch (c) {
    case 'h':
      fprintf(stdout, ""  " "  ". Transforms bitmaps into vector graphics.\n\n");
      usage(stdout);
      exit(0);
      break;
    case 'v':
    case 'V':
      fprintf(stdout, ""  " "  ". Copyright (C) 2001-2019 Peter Selinger.\n");
	...
	break;
      ...
      if (matches == 0) {
	fprintf(stderr, ""  ": unrecognized page format -- %s\n", optarg);
      } else {
	fprintf(stderr, ""  ": ambiguous page format -- %s\n", optarg);
      }
    ...
    case '3':
      ...
      info.compress = 1;
#else
      fprintf(stderr, ""  ": option -3 not supported, using -2 instead.\n");
      fflush(stderr);
      info.pslevel = 2;
      info.compress = 1;
#endif
	...
	if (r==1) {
	  ...
	  fprintf(stderr, ""  ": ambiguous backend -- %s\n", optarg);
	}
	j = fprintf(stderr, "Use one of: ");
	...
	break;
      ...
      if (matches == 0) {
	fprintf(stderr, ""  ": unrecognized turnpolicy -- %s\n", optarg);
      } else {
	fprintf(stderr, ""  ": ambiguous turnpolicy -- %s\n", optarg);
      }
      j = fprintf(stderr, "Use one of: ");
      for (i=0; turnpolicy[i].name!=NULL; i++) {
    ...
    case 'a':
      ...
      if (*p) {
	fprintf(stderr, "  : invalid alphamax -- %s\n", optarg);
	exit(1);
      }
      break;
    case 'O':
      ...
      if (*p) {
	fprintf(stderr, "  : invalid opttolerance -- %s\n", optarg);
	exit(1);
      }
      break;
    case 'o':
    ...
    case 'k':
      ...
      if (*p) {
	fprintf(stderr, "  : invalid blacklevel -- %s\n", optarg);
	exit(1);
      }
      break;
    case 'i':
	...
	fprintf(stderr, "  : invalid tty mode -- %s. Try --help for more info\n", optarg);
	exit(1);
      }
      break;
    case '?':
...
static void process_file(backend_t *b, const char *infile, const char *outfile, FILE *fin, FILE *fout) { 
  ...
  for (count=0; ; count++) {
    ...
    case -4:  /* wrong magic */
      if (count>0) { 
	fprintf(stderr, "  : %s: warning: junk at end of file\n", infile);
	return;
      }
...
int main(int ac, char *av[]) {
  ...
  b = info.backend;
  if (b==NULL) {
    fprintf(stderr, "  : internal error: selected backend not found\n");
    exit(1);
  }
  ...
  */
  ...
  if (!info.some_infiles) {                 /* read from stdin */
    ...
    if (!b->multi && info.infilecount >= 2) {
      fprintf(stderr, "  : cannot use multiple input files with -o in %s mode\n", b->name);
      exit(1);
    }
    if (info.infilecount == 0) {
      fprintf(stderr, "  : cannot use empty list of input files with -o\n");
      exit(1);
    }
